...
class Graph:
    ...
    def Compress(self, iteration, pattern):
        ...
        for instance in pattern.instances:
            ...
            newVertexLabel = 'PATTERN_' + str(iteration)
            newVertexId = 'PATTERN_' + str(iteration) + '_' + str(instanceNum)
    ...
    def write_to_dot_old(self, outputFileName):
    ...
    def write_to_dot(self, outputFileName):
        ...
        for vertex in self.vertices.itervalues():
            ...
            if ('label' in vertex.attributes):
                labelStr = str(vertex.attributes['label'])
            #if ('line' in vertex.attributes):
                #lineNr = str(vertex.attributes['line'])

            typeStr=""
            shapeStr="box, style=filled,color=lightgrey"
            if ('type' in vertex.attributes):
                typeStr = str(vertex.attributes['type'])

            if (typeStr=="VarNode"):
                shapeStr="oval,style=dotted,color=red"
            outputFile.write(str(vertex.id) + ' [label=' + labelStr + ', shape=' + shapeStr + '];\n')
        ...
        for edge in self.edges.itervalues():
            ...
            outputStr = str(edge.source.id) + ' -> ' + str(edge.target.id) + '['   # + ' [label=' + labelStr
            if (not edge.directed):
                outputStr += 'dir=forward'
            if(labelStr=="UseEdge"):
                outputStr += ',color=green'
            if (labelStr == "DefEdge"):
                outputStr += ',color=blue'
            if (labelStr == "CfgEdge"):
                outputStr += ',color=black'
            outputStr += '];\n'
            outputFile.write(outputStr)
        outputFile.write('}\n')
        outputFile.close()
